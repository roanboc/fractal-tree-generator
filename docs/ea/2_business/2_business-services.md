...
| **3D tree beholding**     | Creator         | Grow the chapter-3 tree in three dimensions — tilt **and** twist per split — and orbit it by hand           | 3D tree service (`Tree3DService`) + WebGL renderer | `tree3d.html`       |
...
- _3D tree beholding_ deliberately offers **fewer knobs** than _Tree
  generation_ (single-value sliders, no ranges): the chapter's payoff is
  orbiting the shape, not parameter mastery — the visitor already earned
  that in chapter 3. Interaction (drag to orbit, zoom, slow spin) lives in
  the renderer adapter, not the rule.
