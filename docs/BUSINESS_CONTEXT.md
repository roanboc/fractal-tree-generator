...
_[← README](../README.md) · [ARCHITECTURE](../ARCHITECTURE.md) · [CONTRACTS](./CONTRACTS.md) · [DATA_ARCHITECTURE](./DATA_ARCHITECTURE.md)_

Technical documentation ([ARCHITECTURE.md](../ARCHITECTURE.md),
[CONTRACTS.md](./CONTRACTS.md)) describes _how_ the system is built. This
document describes _why_ — the problem being solved, who it's for, the
vocabulary the code should use consistently, and the reasoning behind the
business rules encoded in `ConfigService`. Keeping this alongside the
...
| Actor                      | Goal                                                                                                        | Touches                                                                 |
| -------------------------- | ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| **End user**               | Explore fractal parameters interactively and get a visually pleasing tree; optionally download it as a PNG. | Deployed static web app only. No account, no data persisted about them. |
| **Developer / maintainer** | Reproduce a specific tree from the command line, batch-generate images, or review past generations.         | CLI, local SQLite database, local `output/`/`logs/` directories.        |
| **Contributor**            | Extend the algorithm or add a new render target (e.g. SVG export) without breaking the other platform.      | `core/` and `adapters/` per [ARCHITECTURE.md](../ARCHITECTURE.md).      |
...
(see [DATA_ARCHITECTURE.md](./DATA_ARCHITECTURE.md#data-classification)).
...
(`ConfigService.validate`, see [CONTRACTS.md](./CONTRACTS.md#iconfigservice))
...
- **Bounded context stays small on purpose.** Resist adding
  ...
  own contracts instead (see [ARCHITECTURE.md](../ARCHITECTURE.md#adding-a-new-platform-eg-a-nativeelectron-renderer)
